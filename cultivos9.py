...
import json
import os
import csv # Módulo para el manejo de archivos CSV

# Módulos para la Interfaz Gráfica de Usuario (GUI)
import tkinter as tk
from tkinter import ttk, messagebox, filedialog # Incluye filedialog para guardar archivos
...
COLOR_FONDO_OSCURO = '#2E3436'     # Gris muy oscuro, base
COLOR_FONDO_SEGUNDARIO = '#343A40' # Gris ligeramente más claro para marcos y entradas
COLOR_TEXTO_CLARO = '#F0F0F0'      # Blanco roto para el texto principal
COLOR_ENFASIS_VERDE = '#5CB85C'    # Verde brillante (Éxito/Venta)
COLOR_ENFASIS_ROJO = '#DC3545'     # Rojo brillante (Pérdida/Costo)
COLOR_PRIMARIO_BOTON = '#007BFF'   # Azul brillante para acción primaria
COLOR_TREE_HEADER = '#495057'      # Gris oscuro para encabezados de tabla
COLOR_ALERTA_CLARA = '#FFC107'     # Amarillo brillante para la alerta de cosecha
...
class AppCultivos(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Asistente de Cultivos (Tema Oscuro + Exportación)")
    ...
    def crear_widgets(self):
        ...
        # Botones de lista (Incluye el nuevo botón de Exportar)
        frame_botones_lista = ttk.Frame(frame_mostrar, style='TLabel') 
        frame_botones_lista.pack(fill='x', pady=5)
        
        # Botón de Exportar (Nuevo)
        ttk.Button(frame_botones_lista, text="📤 Exportar CSV", command=self.exportar_a_csv, style='Principal.TButton').pack(side='left', expand=True, fill='x', padx=5)
    ...
    def exportar_a_csv(self):
        ...
        try:
            with open(nombre_archivo_csv, 'w', newline='', encoding='utf-8') as csvfile:
                # Usamos el punto y coma (;) como delimitador, común en sistemas europeos
